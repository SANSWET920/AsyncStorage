...
TouchableOpacity,AsyncStorage,ImageBackground ,Alert, KeyboardAvoidingView,Share, Clipboard,StyleSheet , Button} from 'react-native';
...
export default function PrivateScreen(props) {
  ...
  const save = async () => {
    try {
      await AsyncStorage.setItem("MyNames1", name);
    } catch (err) {
      alert(err);
    } 
  };
  ...
  const load = async () => {
    try {
      let name = await AsyncStorage.getItem("MyNames1");
  ...
  const remove = async () => {
    try {
      await AsyncStorage.removeItem("MyNames1")
    } catch (err) {
      alert(err)
    } finally {
       setName("")
